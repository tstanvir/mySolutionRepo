...
int cs=0,timer;
vi edg[maxx],st(maxx),en(maxx),depth(maxx);
...
void solve(){
    ...
    vi verOnLevel[n];
    rep1(i,1,n){
    	verOnLevel[depth[i]].pb(st[i]);
    	verOnLevel[depth[i]].pb(en[i]);
    }
    rep1(i,0,n-1){
    	sort(ALL(verOnLevel[i]));
    }
    int q;
    cin>>q;
    rep(i,q){
    	...
    	int lo=lower_bound(ALL(verOnLevel[d]),st[x])-verOnLevel[d].begin();
    	int up=upper_bound(ALL(verOnLevel[d]),en[x])-verOnLevel[d].begin();
    	cout<<(up-lo)/2<<endl;
    }
}
