...
#endif
 int n,m,k;
 vpii vec(105),pep(105);
 int sol(int pos,string s){
    //cout<<pos<<" "<<k<<endl;
    if(pos==k){
        int cnt=0;
        vi mask(n+1);
        rep(i,k){
            if(s[i]=='0') mask[pep[i].ff]=1;
            else mask[pep[i].ss]=1;
        }
        rep(i,m){
    ...
    int res1=sol(pos+1,s+='0');
    s.pop_back();
    int res2=sol(pos+1,s+='1');
    s.pop_back();
