// Problem: E - String Transformation
// Contest: Virtual Judge - Solo Speed Contest - 05
// URL: https://vjudge.net/contest/456314#problem/E
...
void solve(){
    ...
    rep(i,26){
    	if(sz(a[i])){
    		bigA.emplace_back(a[i]);
    	}
    	if(sz(b[i])){
    		bigB.emplace_back(b[i]);
    	}
    }
    sort(ALL(bigA));
    sort(ALL(bigB));
