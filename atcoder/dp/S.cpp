...
#endif
string s;
ll d,dp[10005][2][105],n;
ll sol(ll pos,ll isSmall,ll mod){
    //cout<<pos<<
    if(pos==n) return mod==0;
    ll &res=dp[pos][isSmall][mod];
    if(res!=-1) return res;
    res=0;
    ll lo=0,hi=s[pos]-'0';
    if(isSmall) hi=9ll;
    rep1(i,lo,hi){
        ll newSmall=isSmall | (i<hi);
        res+=sol(pos+1,newSmall,(mod+i)%d);
        res%=MOD;
    }
    return res;
}
...
void solve(){
    ...
    n=sz(s);
    CLR(dp);
    cout<<((sol(0,0,0)-1ll)+MOD)%MOD<<endl;
}
