...
int n;
string s;
ll dp[maxx][10][maxx];//[ith index,type,set of types]
ll sol(int ind,int t,int mask){
	if(ind==n){
		return mask>0;//base case: if there is a single type in the set,
						// return 1. otherwise 0.
	}
	ll &res=dp[ind][t][mask];
	if(res!=-1) return res;
	res=0;
	res+=sol(ind+1,t,mask);//without taking ith indexed type
	normal(res);
	int ch=s[ind]-'A';
	if(ch==t){
		res+=sol(ind+1,t,(mask|(1<<t)));//if previously taken type 
		normal(res);						//matches the ith type, take it
	}
	else if((mask&(1<<ch))==0){
		res+=sol(ind+1,ch,(mask|(1<<ch)));//if previously taken type does
										//not match the ith type and it 
										//does not exist in the set or mask
										//take it.
	}
	normal(res);
	return res;
