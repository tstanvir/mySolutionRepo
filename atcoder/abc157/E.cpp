...
void qu(int ql,int qh,int nodel,int nodeh,int pos){
    if(nodel>nodeh) return ;
    if(ql<=nodel and qh>=nodeh) {
            rep(i,26){
                if(segt[pos][i]) st.insert((char)(i+'a'));;
            }
            return;
    }
    if(ql>nodeh or qh<nodel) return ;
    int mid=(nodel+nodeh)/2;
    qu(ql,qh,nodel,mid,2*pos+1);
    qu(ql,qh,mid+1,nodeh,2*pos+2);
}
