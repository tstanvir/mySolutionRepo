...
void solve(){
    ...
    while(k>0 and !q.empty()){
    	if(q.size()>=2){
    		ll biggest=q.top();
    		q.pop();
    		ll secondBiggest=q.top();
    		ll df=biggest-secondBiggest;
    		ll freq=mm[biggest];
    		mm[biggest]=0;
    		if(df*freq<=k){
    			ll mid=(biggest*(biggest+1))/2ll
    				-(secondBiggest*(secondBiggest+1)/2ll);
    			res+=(mid*freq);
    			k-=df*freq;
    			mm[secondBiggest]+=freq;
    		}
    		else{
    			while(freq<=k){
    				k-=freq;
    		...
    		// ll mn=min(df*freq,k);
    		// //assert(freq!=0);
    		// ll vagfol=mn/freq,vagses=mn%freq;
//     		
    		// ll fst=max(0ll,biggest-vagfol+1);
    		// ll scnd=biggest;
    		// ll num=scnd-fst+1;
    		// res+=1ll*(((fst+scnd)*num)/2ll)*freq;
    		// res+=1ll*((fst-1)*vagses);
    		// //debug(biggest,freq,res);
//     		
    		// k-=mn;
    		// //biggest-=mn;
//     		
    		// if(k>0) mm[secondBiggest]+=freq;
