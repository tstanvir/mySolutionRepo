...
void solve(){
    ...
    rep(i,m){
    	...
    	if(!dq[i].empty() and state[dq[i].front()]!=-1)
    	{
    		removable.emplace_back(state[dq[i].front()],i);
    		state[dq[i].front()]=-1;
    	}
    	else state[dq[i].front()]=i;
    }
    int cnt=0;
    while(!removable.empty()){
    	pii cur=removable.front();
    	//debug(cur.ff,cur.ss);
    	cnt++;
    	removable.pop_front();
    	if(!dq[cur.ff].empty())dq[cur.ff].pop_front();
    	if(!dq[cur.ss].empty())dq[cur.ss].pop_front();
    	if(!dq[cur.ff].empty() and state[dq[cur.ff].front()]!=-1){
    		removable.emplace_back(state[dq[cur.ff].front()],cur.ff);
    		state[dq[cur.ff].front()]=-1;
    	}
    	else if(!dq[cur.ff].empty()) state[dq[cur.ff].front()]=cur.ff;
    	if(!dq[cur.ss].empty() and state[dq[cur.ss].front()]!=-1){
    		removable.emplace_back(state[dq[cur.ss].front()],cur.ss);
    		state[dq[cur.ss].front()]=-1;
    	}
    	else if(!dq[cur.ss].empty())state[dq[cur.ss].front()]=cur.ss;
    }
    if(cnt==n){
    	cout<<"Yes"<<endl;
    }
