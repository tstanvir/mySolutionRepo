...
void solve(){
    ...
    while(!q.empty()){
    	...
    	rep1(i,-2,2)rep1(j,-2,2){
    		int dx=abs(i)+abs(j);
    		if(dx==0 or dx==4) continue;
    		int newx=i+u.ff,newy=j+u.ss;
    		if(border(newx,newy,n,m)){
    			if(s[newx][newy]=='.' and dx==1){
    				if(dist[newx][newy]>dist[u.ff][u.ss]){ 
    				dist[newx][newy]=min(dist[u.ff][u.ss],dist[newx][newy]);
    				q.push_front({newx,newy});
    				}
    				
    			}
    			else if(dist[u.ff][u.ss]+1<dist[newx][newy]){
    				dist[newx][newy]=min(dist[u.ff][u.ss]+1,dist[newx][newy]);
