...
void solve(){
    ...
    rep(i,n){
    	int rw,cl;
    	cin>>rw>>cl;
    	row.emplace_back(rw,i);
    	col.emplace_back(cl,i);
    }
    sort(ALL(row));
    sort(ALL(col));
    vpii ans(n+1);
    ...
    rep(i,n){
    	if(row[i].ff>rw){
    		if(i+1<n and row[i].ff==row[i+1].ff){
    			ans[row[i].ss].ff=rw;
    		}
    		else ans[row[i].ss].ff=rw,rw++;
    	} 
    	else{
    		rw=row[i].ff;
    		ans[row[i].ss].ff=rw;
    ...
    rep(i,n){
    	if(col[i].ff>cl) {
    		if(i+1<n and col[i].ff==col[i+1].ff){
    			ans[col[i].ss].ss=cl;
    		}
    		else ans[col[i].ss].ss=cl,cl++;
    	}
    	else{
    		cl=col[i].ff;
    		ans[col[i].ss].ss=cl;
    ...
    rep1(i,1,n){
    	cout<<ans[i-1].ff<<" "<<ans[i-1].ss<<endl;
    }
}
