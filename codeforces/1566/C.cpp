...
void solve(){
    ...
    string s,t;
    cin>>s>>t;
    vpii equalZero,equalOne,notEqual;
    int cnt=0;
    int ans=0;
    rep(i,n){
    	if(s[i]!=t[i]){
    		notEqual.emplace_back(i,i);
    		if(sz(equalZero)){
    			ans+=(int)equalZero.size();
    			equalZero.clear();
    		}
    		equalOne.clear();
    	}
