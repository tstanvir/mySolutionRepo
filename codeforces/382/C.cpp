...

{
    ...
    while(cin>>n){
        vll vec(n);
        rep(i,n) cin>>vec[i];
        ll maxi=maxv(vec),mini=minv(vec);
        set<int>con(ALL(vec));
        if(n==1) cout<<-1<<endl;
        else if(con.size()==1){
            cout<<1<<endl;
            ...
            if((vec[0]+vec[1])%2==0){
                ...
                cout<<mini-diff<<" "<<(maxi+mini)/2<<" "<<maxi+diff<<endl;
            }
            else{
                cout<<2<<endl;
                cout<<mini-diff<<" "<<maxi+diff<<endl;
            }
        }
        else{
            sort(ALL(vec));
            ...
            if(cnt==1){
                ...
                rep1(i,1,n-1){
                    if(vec[i]-vec[i-1]==val){
                        if((vec[i-1]+(val))==vec[i] && *(st.begin())!=0 && ((val/2)==*(st.begin()))){
                            ans+=(vec[i-1]+(val/2));
                            bb=true;
                            break;
                        }
