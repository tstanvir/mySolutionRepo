...
// URL: https://codeforces.com/problemset/problem/1417/C
// Memory Limit: 256 MB
// Time Limit: 1000 ms
// 
...
void solve(){
    ...
    vi gap(n+1);
    rep1(i,1,n){
    	gap[vec[i]]=max(gap[vec[i]],i-last[vec[i]]-1);
    	last[vec[i]]=i;
    }
    rep1(i,1,n){
    	gap[i]=max(gap[i],n-last[i]);
    }
    vi optimalK(n+2,(int)1e8);
    rep1(i,1,n){
    	optimalK[gap[i]+1]=min(optimalK[gap[i]+1],i);
    }
    int pre=n+1;
    rep1(i,1,n){
    	pre=min(pre,optimalK[i]);
    	if(pre==n+1) cout<<-1<<" ";
    	else cout<<pre<<" ";
    }
    cout<<endl;
