...
void solve(){
    ...
    /*multiset<ll>mm;
    mm.insert(vec[n-1].ff);
    irep(i,n-2,0){
        if(i>0){
            ll val=vec[i].ff+pre[i-1];
            //pair<ll,ll>p={val,-1};
            if(mm.!=sortedV.begin())
            {
                ans.pb(vec[i].ss);
            ...
            ll val=vec[i].ff;
            if(upper_bound(ALL(sortedV),val)!=sortedV.begin())
            {
                ans.pb(vec[i].ss);
            }
        }
    ...
    /*irep(i,n-2,0){
        if(i>0){
            if(vec[i].ff+pre[i-1]>=vec[i+1].ff){
                ans.pb(vec[i].ss);
            }
        }
        else {
