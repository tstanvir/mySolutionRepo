...
void solve(){
    ...
    ll boySt[n+2][m+2],boyEn[n+2][m+2],girlSt[n+2][m+2],girlEn[n+2][m+2];
    SET(boySt),SET(boyEn),SET(girlSt),SET(girlEn);
    rep1(i,1,n){
        rep1(j,1,m){
            boySt[i][j]=max(boySt[i-1][j],boySt[i][j-1])+cost[i][j];
        }
    }
    irep(i,n,1)
    {
        irep(j,m,1){
            boyEn[i][j]=max(boyEn[i+1][j],boyEn[i][j+1])+cost[i][j];
        }
    }
    irep(i,n,1){
        rep1(j,1,m){
            girlSt[i][j]=max(girlSt[i+1][j],girlSt[i][j-1])+cost[i][j];
        }
    }
    rep1(i,1,n){
        irep(j,m,1){
            girlEn[i][j]=max(girlEn[i-1][j],girlEn[i][j+1])+cost[i][j];
        }
    }
    ll ans=0;
    rep1(i,2,n-1){
        rep1(j,2,m-1){
            ll op1=boySt[i][j-1]+boyEn[i][j+1]+girlSt[i+1][j]+girlEn[i-1][j];
            ll op2=boySt[i-1][j]+boyEn[i+1][j]+girlSt[i][j-1]+girlEn[i][j+1];
            ans=max({ans,op1,op2});
        }
    }
    cout<<ans<<endl;
