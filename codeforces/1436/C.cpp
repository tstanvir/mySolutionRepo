...
void solve(){
    ...
    int l=0,r=n;
    //vll ans(n+1,0);
    ll boro=n-x,choto=x-1,cntchoto=0,cntboro=0;
    while(l<r){
    ...
    ll baki=(n-(cntboro+cntchoto+1));
    ll ans=(ncr(boro,cntboro)%MOD*fact[cntboro]%MOD)%MOD;
    ans*=(ncr(choto,cntchoto)%MOD*fact[cntchoto]%MOD)%MOD;
    ans%=MOD;
    ans*=fact[baki];
    ans%=MOD;
    cout<<ans<<endl;
