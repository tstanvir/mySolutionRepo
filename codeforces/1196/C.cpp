...
bool sortinrev(const pair<pair<int,int>,int> &a,const pair<pair<int,int>,int> &b)
{
...
void solve(){
    ...
    int n;
    cin>>n;
    vector<pair<pair<int,int>,int>> ini(n);
    int d[n][3];
    //set<int>x,y;
    int maxy=100000,maxx=100000,miny=-100000,minx=-100000;
    rep(i,n){
    	cin>>ini[i].ff.ff>>ini[i].ff.ss>>d[i][0]>>d[i][1]>>d[i][2]>>d[i][3];
    	ini[i].ss=i;
    	if(d[i][1]==0){
    		maxy=min(maxy,ini[i].ff.ss);
    	}
    	if(d[i][2]==0){
    		maxx=min(maxx,ini[i].ff.ff);
    	}
    	if(d[i][3]==0){
    		miny=max(miny,ini[i].ff.ss);
    	}
    	if(d[i][0]==0){
    		minx=max(minx,ini[i].ff.ff);
    	}
    }
