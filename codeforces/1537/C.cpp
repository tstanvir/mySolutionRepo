...
void solve(){
    ...
    vi vec(n);
    map<int,int>mm;
    rep(i,n) cin>>vec[i],mm[vec[i]]++;
    
    	sort(ALL(vec));
    	pll p;
    	int maxi=highest(int);
    	rep(i,n-1){
    		if(abs(vec[i]-vec[i+1])<maxi){
    			maxi=abs(vec[i]-vec[i+1]);
    			p={i,i+1};
    		}
    	}
    	//debug(vec[p.ff],vec[p.ss]);
    	int val=min(vec[p.ff],vec[p.ss]);
    	mm[val]--;
    	mm[max(vec[p.ff],vec[p.ss])]--;
    	cout<<min(vec[p.ff],vec[p.ss])<<" ";
    	forch(it,mm){
    		if(it.ss>0 and it.ff>=val){
    			int cnt=mm[it.ff];
    			while(cnt){
    				cout<<it.ff<<" ";
    				mm[it.ff]--;
    	...
    	cout<<max(vec[p.ff],vec[p.ss])<<endl;
...
int maxi=maxv(vec);
    	...
    	forch(it,mm){
    		if(it.ss>0 and it.ff<=val){cout<<it.ff<<" ";mm[it.ff]--;}
    	}
    	cout<<val<<" ";
    	forch(it,mm){
    		if(it.ss>0) cout<<it.ff<<" ";
    	}
    	cout<<endl;
    	*/