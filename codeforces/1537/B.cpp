...
int cs=0;
ll dist(pll a,pll b,pll c){
	ll ans=0;
	ans+=(abs(a.ff-b.ff)+abs(a.ss-b.ss));
	ans+=(abs(a.ff-c.ff)+abs(a.ss-c.ss));
	ans+=(abs(c.ff-b.ff)+abs(c.ss-b.ss));
	return ans;
}
void solve(){
    //cout<<"Case "<<++cs<<": ";
    ...
    forch(it,vec){
    	forch(jt,vec){
    		if(dist(it,jt,{inix,iniy})>maxi){
    			//debug(it.ff,it.ss,jt.ss,jt.ss);
    			maxi=dist(it,jt,{inix,iniy});
    			ans={it,jt};
    		}
    	}
    }
