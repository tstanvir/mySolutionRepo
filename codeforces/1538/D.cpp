...
void solve(){
    ...
    int n=sz(primes);
    ll costA=0,costB=0;
    for(int i=0;i<n and primes[i]*primes[i]<=aa;i++){
    	while(aa%primes[i]==0){
    		costA++;
    		aa/=primes[i];
    	}
    ...
    if(aa>1) costA++;
    for(int i=0;i<n and primes[i]*primes[i]<=bb;i++){
    	while(bb%primes[i]==0){
    		costB++;
    		bb/=primes[i];
    	}
    ...
    if(bb>1) costB++;
    int val=gc==min(a,b)?1:2;
    if(a==b and k==1){
    	cout<<"NO"<<endl;
    	return;
    }
    if(k>=val and k<=(costA+costB)){
    	cout<<"YES"<<endl;
    }
    else cout<<"NO"<<endl;
}
