...
void solve(){
    ...
    rep(i,n){
        rep1(j,i+1,n-1){
            rep1(k,j+1,n-1){
                if(max({vec[i],vec[j],vec[k]})-min({vec[i],vec[j],vec[k]})<=2) {
                    debug(vec[i],vec[j],vec[k]);
                }
            }
        }
    }*/
    ...
    rep(i,szz){
        ...
        ans+=freq[x]*(freq[x+1]*(freq[x+1]-1))/2;
        ans+=freq[x]*(freq[x+2]*(freq[x+2]-1))/2;
        ans+=((freq[x]*(freq[x]-1))/2)*freq[x+1];
        ans+=((freq[x]*(freq[x]-1))/2)*freq[x+2];
