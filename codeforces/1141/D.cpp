...
void solve(){
    ...
    vector<int>ind[2][27];
    rep(i,n){
    	if(l[i]=='?') ind[0][26].pb(i+1);
    	else ind[0][l[i]-'a'].pb(i+1);
    }
    rep(i,n){
    	if(r[i]=='?') ind[1][26].pb(i+1);
    	else ind[1][r[i]-'a'].pb(i+1);
    }
    vpii ans;
    rep(i,26){
    	while(sz(ind[0][i]) and sz(ind[1][i])) ans.pb({ind[0][i].back(),ind[1][i].back()}),ind[0][i].ppb(),ind[1][i].ppb();
		
    }
    rep(i,26){
    	while(sz(ind[0][26]) and sz(ind[1][i])) ans.pb({ind[0][26].back(),ind[1][i].back()}),ind[0][26].ppb(),ind[1][i].ppb();
		
    }
    rep(i,26){
    	while(sz(ind[0][i]) and sz(ind[1][26])) ans.pb({ind[0][i].back(),ind[1][26].back()}),ind[0][i].ppb(),ind[1][26].ppb();
    }
    while(sz(ind[0][26]) and sz(ind[1][26])) ans.pb({ind[0][26].back(),ind[1][26].back()}),ind[0][26].ppb(),ind[1][26].ppb();
	cout<<sz(ans)<<endl;
	forch(it,ans){
