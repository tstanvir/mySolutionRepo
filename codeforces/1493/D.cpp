...
int cs=0;
multiset<int>Fact[maxx+1];
map<int,int>mmFact[maxx+1];
ll ans=1;
int n,q;
int spf[maxx];
void sieve(){
...
void update(int ind,int val){
    while(val!=1){
        int div=spf[val],cnt=0;
        while(spf[val]==div) cnt++,val/=spf[val];
        int last=mmFact[ind][div];
        mmFact[ind][div]+=cnt;
        int mini=0;
        if((int)Fact[div].size()==n){
            mini=*(Fact[div].begin());
        }
        if(last!=0){
            Fact[div].erase(Fact[div].find(last));
        }
        Fact[div].insert(last+cnt);
        if((int)Fact[div].size()==n){
            for(int j=mini+1;j<=*Fact[div].begin();j++){
                ans=modMul(ans,(div)*1ll);
                normal(ans);
            }
        }
