...
void solve(){
    ...
    rep(i,n){
    	cin>>vec[i];
    	if(i%2==0) odd.insert(vec[i]),sumOdd+=vec[i];
    	else even.insert(vec[i]),sumEven+=vec[i];
    }
    ll mini=highest(ll);
    int komche=0;
    irep(i,n-1,1){
    	ll val=odd.size();
    	ll cost=(sumOdd-(*odd.begin()))+(*odd.begin())*(n-(val-1));
    	//debug(cost,n,i);
    	val=even.size();
    	cost+=(sumEven-(*even.begin()))+(*even.begin())*(n-(val-1));
    	if(i%2==0){
    		odd.erase(odd.lower_bound(vec[i]));
    		sumOdd-=vec[i];
    	}
    	else even.erase(even.lower_bound(vec[i])),sumEven-=vec[i];
    	mini=min(mini,cost);
    	komche++;
    }
    cout<<mini<<endl;
