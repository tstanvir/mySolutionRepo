...
void solve(){
    ...
    ll mex=0,maxi=-1;
    multiset<ll>mst,mx;
    rep1(i,0,n) mx.insert(i);
    rep(i,n){
        ll x;
        cin>>x;
        maxi=max(maxi,x);
        if(mx.find(x)!=mx.end()) mx.erase(x);
        mst.insert(x);
    }
    mex=*mx.begin();
    ll val=(mex+maxi+1)/2ll;
    if(val>maxi) cout<<n+k<<endl;
    else if(val<maxi and mst.find(val)==mst.end() and k>=1) cout<<n+1<<endl;
    else cout<<n<<endl;
