...
void dfs(ll child,ll par){
    forch(it,edg[child]){
        if(it!=par){
            //parent[it]=par;
            dfs(it,child);
            dp[child][0]+=max(dp[it][0]+abs(le[child]-le[it]),dp[it][1]+abs(le[child]-ri[it]));
            dp[child][1]+=max(dp[it][0]+abs(ri[child]-le[it]),dp[it][1]+abs(ri[child]-ri[it]));    
        }
    }
}
void init(int n){
    ...
    //fill(le.begin(),le.begin()+n,0);
    //fill(ri.begin(),ri.begin()+n,0);
    SET(le);
    SET(ri);
}
void solve(){
