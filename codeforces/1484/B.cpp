...
void solve(){
	...
	set<ll>pos,neg;
	rep1(i,1,n-1){
		if(vec[i]-vec[i-1]>=0) pos.insert(vec[i]-vec[i-1]);
		else neg.insert((vec[i]-vec[i-1]));
	}
	if(sz(pos)>1 or sz(neg)>1) {
		cout<<-1<<endl;
		return;
	}
	if(sz(pos)==0 or sz(neg)==0) {
		cout<<0<<endl;
		return;
	}
	ll ans=*pos.begin()+(-1*(*neg.begin()));
	if(ans<=maxv(vec)){
		cout<<-1<<endl;
		return;
	}
	cout<<ans<<" "<<*pos.begin()<<endl;
