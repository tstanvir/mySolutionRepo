...
void solve(){
    ...
    int a=0,b=0;
	ll sum=0;
	while(sz(both) and sz(onlyA) and sz(onlyB) and (a<k or b<k)){
		if(both.front()<=onlyA.front()+onlyB.front()){
			a++;
			b++;
			sum+=both.front();
			both.pop_front();
			...
			a++;
			b++;
			sum+=onlyA.front()+onlyB.front();
			onlyA.pop_front(),onlyB.pop_front();
		}
	}    
	while((a<k or b<k) and sz(both)){
		sum+=both.front();
		both.pop_front();
		a++,b++;
	}
	while(sz(onlyA) and a<k){
		a++;
		sum+=onlyA.front();
		onlyA.pop_front();
	}
	while(sz(onlyB) and b<k){
		b++;
		sum+=onlyB.front();
		onlyB.pop_front();
	}
...
ll sum=0;
    sort(ALL(both));
    while(both.size()>k){both.pop_back();}
    sort(ALL(onlyA));
    sort(ALL(onlyB));
    int a=0,b=0;
    while( sz(both) and sz(onlyA) and sz(onlyB) and both.back()>onlyA.front()+onlyB.front()){
    	both.ppb();
    	sum+=onlyA.front()+onlyB.front();
    	a++,b++;
    	onlyA.pop_front();
    ...
    sum+=(accumulate(ALL(both),0));
    a+=sz(both);
    b+=sz(both);
    if(a<k){
    	rep(i,min(k-a,sz(onlyA))){
    		sum+=onlyA[i];
    	}
    	a+=min(k-a,int(sz(onlyA)));
    	if(a<k){
    		cout<<-1<<endl;
    		return;
    	}
    ...
    if(b<k){
    	rep(i,min(k-b,sz(onlyB))){
    		sum+=onlyB[i];
    	}
    	b+=min(k-b,int(sz(onlyB)));
    	if(b<k){
    		cout<<-1<<endl;
    		return;
    	}
