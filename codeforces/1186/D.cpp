// Problem: D. Vus the Cossack and Numbers
...
void solve(){
    ...
    rep(i,n){
    	...
    	if(x<0){
    		x*=-1.0;
    		negSum+=(int)x;
    		neg.pb({i,{int(x),int(ceil(x))}});
    	}
    	else{
    		posSum+=(int)x;
    		pos.pb({i,{int(x),int(ceil(x))}});
    	}
    }
   // debug(negSum,posSum);
    if(negSum>posSum){
    	int val=negSum-posSum;
    	int i=0;
    	int szz=sz(pos);
    	while(val>0 and i<szz){
    		if(pos[i].ss.ff!=pos[i].ss.ss) val--;
    		pos[i].ss.ff=pos[i].ss.ss;
    	...
    	vi ans(n);
    	 szz=sz(pos);
    	rep(i,szz){
    		ans[pos[i].ff]=pos[i].ss.ff;
    	}
    	szz=sz(neg);
    	rep(i,szz){
    		ans[neg[i].ff]=-1*neg[i].ss.ff;
    	}
    	rep(i,n){
    		cout<<ans[i]<<endl;
    	}
    	...
    	int val=posSum-negSum;
    	int i=0;
    	int szz=sz(neg);
    	while(val>0 and i<szz){
    		if(neg[i].ss.ff!=neg[i].ss.ss) val--;
    		neg[i].ss.ff=neg[i].ss.ss;
    		//debug(neg[i].ss.ff,neg[i].ss.ss);
